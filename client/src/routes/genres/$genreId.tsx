...
import { updateGenreSchema, type UpdateGenreDto } from "@/features/genres/schemas.ts";
import { useGenre, useUpdateGenre } from "@/features/genres/genresApi.ts";
import { useEffect } from "react";

function EditGenrePage(): React.JSX.Element {
	const { genreId } = Route.useParams();
	const { data: genre, isLoading, isError } = useGenre(genreId);
	...
	const onSubmit = (data: UpdateGenreDto): void => {
	...
	return (
		<div className="mx-auto max-w-2xl space-y-8 animate-in fade-in duration-500">
			<div>
				...
				<h1 className="text-3xl font-bold text-slate-900">Редагувати жанр</h1>
				<p className="text-slate-500">Оновіть інформацію про жанр</p>
			</div>
